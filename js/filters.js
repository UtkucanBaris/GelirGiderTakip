...
async function initializeFilters() {
    ...
    const allMethods = [...new Set([
        ...settings.incomeMethods,
        ...settings.expenseMethods
    ])].sort();
    ...
    document.getElementById('filterStartDate').value = formatDateForInput(startOfMonth);
    document.getElementById('filterEndDate').value = formatDateForInput(endOfMonth);
...
function renderTransactions() {
    ...
    if (filteredTransactions.length === 0) {
        tableContainer.innerHTML = '<p class="empty-state">Filtrelere uygun işlem bulunamadı.</p>';
        updateTransactionCount();
        return;
    }
        ...
        html += `
            <tr>
                ...
                <td class="action-buttons">
                    <button class="btn-icon btn-edit" onclick="editTransaction(${transaction.id})" title="Düzenle">✏️</button>
                    <button class="btn-icon btn-delete" onclick="deleteTransaction(${transaction.id})" title="Sil">🗑️</button>
                </td>
            </tr>
        `;
    });
